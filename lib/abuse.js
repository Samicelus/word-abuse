...
class Abuse{
    ...
    load_dictionary(){
        let that = this;
        const root = path.join(process.cwd(), this.path);
        const abuse_files = fs.readdirSync(path.join(process.cwd(), this.path), 'utf8');
    ...
    load_meaningless(){
        let that = this;
        const root = path.join(process.cwd(), this.ignore_path);
        const meaningless_files = fs.readdirSync(path.join(process.cwd(), this.ignore_path), 'utf8');
